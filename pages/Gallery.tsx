...
const Gallery: React.FC = () => {
  ...
  const categories = ['All', 'Kids Braids', 'Lashes', 'Makeup'];
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-16">
          {categories.map((cat) => (
            <button
              ...
              className={`px-8 py-2 font-sans text-[10px] tracking-[0.3em] uppercase border transition-all ${
                filter === cat ? 'bg-brand-espresso text-brand-offwhite border-brand-espresso' : 'border-brand-gold/30 text-brand-espresso hover:border-brand-gold'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
          {[...Array(12)].map((_, i) => (
            <div key={i} className="break-inside-avoid relative group overflow-hidden shadow-sm">
              <img 
                src={`https://picsum.photos/seed/gallery${i}/800/${600 + (i % 3) * 200}`} 
